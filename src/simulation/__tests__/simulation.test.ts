...
describe("simulateSplitterMerger", () => {
  ...
  it("splits 32-bit value to 8-bit outputs", () => {
    ...
    // LSB-aligned: [7:0], [15:8], [23:16], [31:24]
    expect(result.get("out7:0")).toBe(0x78n);
    expect(result.get("out15:8")).toBe(0x56n);
    expect(result.get("out23:16")).toBe(0x34n);
  ...
  it("merges 8-bit inputs to 32-bit output", () => {
    ...
    const result = simulateSplitterMerger(
      { inputBitWidth: 8, outputBitWidth: 32 },
      inputs,
    );
  ...
  it("handles passthrough (same bit widths)", () => {
    const inputs = new Map([["in", 0xabn]]);
    const result = simulateSplitterMerger(
      { inputBitWidth: 8, outputBitWidth: 8 },
      inputs,
    );

    expect(result.get("out7:0")).toBe(0xabn);
  });
});
